...
def STE_cutter_from_file(input_file, frame_size=256, hop_size=128, threshold=0.05):
    ...
    sf.write("record_word/audio_trimmed.wav", non_speech_signal, sample_rate)
...
def STE_cutter(input_file, frame_size=256, hop_size=128, threshold=0.05):
    ...
    sf.write("record_word/audio_temp.wav", speech_signal, sample_rate)
    predicted_class = classify_audio_file("record_word/audio_temp.wav", trained_models)
    ...
    for start, end in speechs_segment:
        ...
        speech_signal = signal[start * hop_size:end * hop_size]
        sf.write("record_word/audio_temp.wav", speech_signal, sample_rate)
        # STE_cutter_from_file("record_word/audio.wav")
        predicted_class = classify_audio_file("record_word/audio_temp.wav", trained_models)
