...
def pad(x, max_len=64600):
    ...
    return padded_x
def transform_pipeline(x):
        x= pad(x)

        return x

def read_file(file_path):
        data_x, sample_rate = sf.read(file_path)
        data_x = transform_pipeline(data_x)
        return data_x
def predict_label(batch_x, model, device):
    val_loss = 0.0
    ...
    print(batch_out)
    first_element = batch_out[0][0].item()
    second_element = batch_out[0][1].item()
    if first_element > second_element:
        confident=(first_element - second_element)/11.5*50+50
        if confident > 100:
            confident = 99.99
    else: 
        confident=(second_element - first_element)/11.5*50+50
        if confident > 100:
            confident = 99.99
    ...
    return confident,_, predicted
def predict(file_name):
    #device
    # device = 'cuda' if torch.cuda.is_available() else 'cpu'
    device = 'cpu'
    print('Device: {}'.format(device))
    #load file cau hinh model
    dir_yaml = os.path.splitext('ai\model_config_RawGAT_ST')[0] + '.yaml'
    ...
    #load model
    model_path="ai\epoch_14_newbest.pth"
    if model_path:

        model.load_state_dict(torch.load(model_path,map_location=device))
        print('Model loaded : {}'.format(model_path))
    x=read_file(file_name)
    tensor_result = torch.tensor([x], dtype=torch.float32)
    y=predict_label(tensor_result, model, device)
    print(y)
    if(y[2].item()==1):
        return "Confident: "+str(round(y[0],2)),"bonfide"
    else:
        return "Confident: "+str(round(y[0],2)),"spoof"